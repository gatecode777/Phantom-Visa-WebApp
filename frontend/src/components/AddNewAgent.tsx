...
export default function AddNewAgent({ onSuccess }: AddNewAgentProps) {
  ...
  const handleClearAllVisaCountries = () => {
    setFormData((prev) => ({ ...prev, supportedVisaCountries: [] }));
    triggerToast("Cleared all selected visa countries.");
  };
  ...
  const validateSinglePhoneField = (fieldName: "phone" | "altPhone", val: string, dialCode: string) => {
    ...
    const raw = val ? val.replace(/\D/g, "") : "";

    if (!raw) {
      if (fieldName === "phone") {
        return `Phone number is required for ${selectedCountry.name}.`;
      }
      return ""; // Optional altPhone when empty
    }
  ...
  const handlePhoneDialCodeChange = (newDialCode: string) => {
    ...
    const truncated = formData.phone ? formData.phone.slice(0, selectedCountry.maxPhoneDigits) : "";
  ...
  const handleAltPhoneDialCodeChange = (newDialCode: string) => {
    ...
    const selectedCountry = COUNTRY_DIAL_CODES.find((c) => c.dialCode === newDialCode) || COUNTRY_DIAL_CODES[0];
    const truncated = formData.altPhone ? formData.altPhone.slice(0, selectedCountry.maxPhoneDigits) : "";
    setFormData((prev) => ({ ...prev, altPhone: truncated }));
    if (!truncated) {
      setErrors((prev) => {
        const copy = { ...prev };
        delete copy.altPhone;
        return copy;
      });
    } else {
  ...
  const handleInputChange = (field: string, value: any) => {
    ...
    if (field === "altPhone") {
      ...
      sanitizedVal = value ? value.replace(/\D/g, "").slice(0, selectedAltCountry.maxPhoneDigits) : "";
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-2xs space-y-4">
            ...
            <div className="space-y-4 text-xs">
              ...
              {/* SUPPORTED VISA COUNTRIES (MULTIPLE CHOICE SELECTOR) */}
              <div className="pt-4 border-t border-slate-100 space-y-3">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                  <div>
                    <label className="text-[10px] font-extrabold uppercase tracking-wider text-slate-700 block flex items-center gap-1.5 font-outfit">
                      <Globe size={14} className="text-[#2563EB]" />
                      <span>Supported Visa Countries (Which Country Visas Agent Provides)</span>
                      <span className="text-red-500">*</span>
                    </label>
                    <p className="text-[11px] text-slate-500 mt-0.5">
                      Select all destination countries for which this agent provides visa application services.
                    </p>
                  </div>
                  ...
                  <div className="flex items-center gap-1.5 shrink-0">
                    <button
                      type="button"
                      onClick={handleSelectPopularVisaCountries}
                      className="px-2.5 py-1 text-[10px] font-bold bg-blue-50 hover:bg-blue-100 text-[#2563EB] rounded-lg transition border border-blue-200 cursor-pointer shadow-2xs"
                    >
                      Top 10 Popular
                    </button>
                    <button
                      type="button"
                      onClick={handleSelectAllVisaCountries}
                      className="px-2.5 py-1 text-[10px] font-bold bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-lg transition border border-slate-200 cursor-pointer shadow-2xs"
                    >
                      Select All ({ALL_VISA_DESTINATION_COUNTRIES.length})
                    </button>
                    <button
                      type="button"
                      onClick={handleClearAllVisaCountries}
                      className="px-2 py-1 text-[10px] font-bold text-red-600 hover:bg-red-50 rounded-lg transition cursor-pointer"
                    >
                      Clear
                    </button>
                  </div>
                ...
                {/* Country Search Bar & Selected Pill Badges */}
                <div className="space-y-2">
                  <div className="relative">
                    <Search size={14} className="absolute left-3 top-2.5 text-slate-400" />
                    <input
                      type="text"
                      placeholder="Search country by name to quickly filter choices..."
                      value={countrySearchQuery}
                      onChange={(e) => setCountrySearchQuery(e.target.value)}
                      className="w-full bg-slate-50 border border-slate-200 rounded-xl pl-9 pr-8 py-1.5 text-xs text-slate-800 focus:outline-none focus:border-[#2563EB] focus:bg-white transition"
                    />
                    {countrySearchQuery && (
                      <button
                        type="button"
                  ...
                  <div className="p-2.5 bg-slate-50 border border-slate-200 rounded-xl min-h-[42px] max-h-28 overflow-y-auto [scrollbar-width:thin] flex flex-wrap gap-1.5 items-center">
                    {(formData.supportedVisaCountries || []).length === 0 ? (
                      <span className="text-[11px] text-slate-400 italic font-medium px-1">
                        No countries selected yet. Click options below to choose which visas this agent provides.
                      </span>
                    ) : (
                      (formData.supportedVisaCountries || []).map((cName) => {
                        ...
                        return (
                          <span
                            key={cName}
                            className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-white border border-blue-200 text-[#2563EB] text-[11px] font-bold shadow-2xs animate-in fade-in duration-100"
                          >
                            {renderCountryFlag(countryObj?.flag)}
                            <span>{cName}</span>
                            <button
                              type="button"
                ...
                {/* Multiple Choice Grid of All Countries */}
                        ...
                        return (
                          <div
                            ...
                            className={`p-2 rounded-xl border text-[11px] font-semibold flex items-center justify-between cursor-pointer select-none transition ${
                              isChecked
                                ? "bg-blue-50/90 border-[#2563EB] text-[#2563EB] ring-1 ring-[#2563EB]/20 font-bold shadow-2xs"
